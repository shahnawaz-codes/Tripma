    ...
    <div className="w-full h-[calc(100vh-70px)] overflow-hidden">
      <ResizablePanelGroup orientation="horizontal" className="w-full h-full">
        ...
        <ResizablePanel
          defaultSize={60}
          minSize={30}
          className="hidden lg:block bg-white dark:bg-neutral-950 overflow-hidden h-full relative"
        >
          {isGeneratingPlan ? (
            <TripStatusDisplay status="generating" />
          ) : tripPlanInfo ? (
            isGlobe ? (
            ...
            <TripStatusDisplay status="empty" />
          )}

          {/* Floating glassmorphic toggle button with hover-card details */}
